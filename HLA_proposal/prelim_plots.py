...
def make_plots(matchcat):
    ...
    ncmap = plt.cm.viridis
    norm = colors.BoundaryNorm(numpy.arange(0, 10, 2), ncmap.N)
    
    plt.figure(figsize=(12,9))
    plt.scatter(Mstar, sSFR, c=n, cmap=ncmap, norm=norm, s=32)
    #plt.title('SFG: {}, GV: {}, Q: {}'.format(len(SFG_Mstar), len(GV_Mstar), len(RS_Mstar)))
    plt.xlabel('log $M_{*}$')
    plt.xlim(7.8, 12.5)
    plt.ylim(-14, -7.8)
    plt.axhline(-10.8, color='k', linestyle='--', linewidth=2)
    plt.axhline(-11.8, color='k', linestyle='--', linewidth=2)
    plt.ylabel('log sSFR')
    plt.colorbar(label='n (SDSS)')
    ax = plt.gca()
    ax.xaxis.set_minor_locator(AutoMinorLocator())
    ax.yaxis.set_minor_locator(AutoMinorLocator())
    ax.xaxis.set_tick_params(direction='in', size=24, top=True)
    ax.yaxis.set_tick_params(direction='in', size=24, right=True)
    ax.xaxis.set_tick_params(direction='in', size=12, top=True, bottom=True, which='minor')
    ax.yaxis.set_tick_params(direction='in', size=12, right=True, left=True, which='minor')
    plt.text(s='Star Forming', x=0.65, y=0.9, size=23, c='b', transform=ax.transAxes)
    plt.text(s='Green Valley', x=0.02, y=0.4, size=23, c='g', transform=ax.transAxes)
    plt.text(s='Quenched', x=0.025, y=0.25, size=23, c='r', transform=ax.transAxes)
    plt.savefig('Plots/sSFR_v_Mstar_ncolor_{}.pdf'.format(matchcat), format='pdf', dpi=500, bbox_inches='tight')
    plt.show()
    plt.close()
