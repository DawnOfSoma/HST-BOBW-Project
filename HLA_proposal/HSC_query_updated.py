...
def main():
    ...
    subsample = [1237653665795801308, 1237654670814675063, 1237655349431959652, 1237658629695799458, 1237661416602534162, 1237674478123417869]
    ...
    for i in range(0,len(n)):
        ...
        if sdss_id not in used_IDs and sdss_id in subsample:
            ...
            while not found_image:
                ...
                #print(image.size, all_pixels.size, bad_pixels.size)
                
                """plt.figure(figsize=(12,9))
                plt.title('z = {}, sSFR = {}, n = {}'.format(round(mz,2), round(mSSFR,2), round(mn,2)))
                plt.imshow(np.arcsinh(image),cmap="gray")
            ...
            #print(pfilter, best_xt, mra, mdec)
            if found_image:
                ...
                while not download_successful:
                    try:
                        image = get_hla_cutout(imname,mra,mdec,size=imsize, autoscale=99.35)
                        #image = np.asarray(get_hla_cutout(imname,mra,mdec,size=imsize,autoscale=100,asinh=False))
                        download_successful = True
                    except:
                        download_successful = False
                ...
                plt.figure(figsize=(12,9))
                plt.title('z = {}, log $M_*$ = {}, log sSFR = {}, n = {}'.format(round(mz,2), round(mMstar,1), round(mSSFR,1), round(mn,1)), size=24)
                ...
                plt.savefig(outpath + '{}_HST.png'.format(sdss_id), bbox_inches = "tight")
                plt.show()
                plt.close()
                
                # Download SDSS image
                SDSS_pixScale = 0.396127
                imsize_SDSS = int( round(4 * prad[i] / SDSS_pixScale) )
                ...
                """plt.figure(figsize=(12,9))
                plt.title('z = {}, sSFR = {}, n = {}'.format(round(mz,2), round(mSSFR,2), round(mn,2)))
                plt.imshow(block_reduce(image, int(SDSS_pixScale/pixScale)), cmap="gray")
                ax = plt.gca()
                ax.xaxis.set_visible(False)
                ax.yaxis.set_visible(False)
                plt.show()
                ...
                while not download_successful:
                    try:
                        ...
                        for path in paths:
                            ...
                            plt.axhline(y=0.85*imsize_SDSS, xmin=0.1, xmax=xmax, color='#ffff00', linewidth=6)
                            plt.text()
                            plt.savefig(outpath + '{}_SDSS.png'.format(sdss_id), bbox_inches = "tight")
                            plt.show()
                            plt.close()
                        download_successful = True
                    except:
    ...
    print(has_good_image_count, total_count)
