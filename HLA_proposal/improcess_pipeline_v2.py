...
def getskymap(image, segmap, pixScale):
...
def process(imname, outfile, exptime, ra, dec, imfilename):
    ...
    segmap = deblend_sources(image_bgsub, segmap,
                                   npixels=100, nlevels=32, contrast=0.00005,
                                   progress_bar=False)
    ...
    infotable = numpy.loadtxt(imname[:-5] + '_info.dat', usecols=(0,1))
    pixScale = infotable[0]
    sky_distance_map, number_of_sky_pixels = getskymap(image, segmap, pixScale)
    ...
    if numpy.isnan(mean) or number_of_sky_pixels < 20000:
        ...
        while loop:
            ...
            imsize = int( round(loop_factor * image.shape[0]) )
            fitsfile = get_hla_cutout(imfilename,ra,dec,size=imsize,autoscale=100,asinh=False)
            
            image_extended = fitsfile[0].data * exptime # electrons / s * exptime
            # need to scale by ncombine and gain if needed
            image_extended = image_extended * ncombine
            if units == 'COUNTS/S':
                image_extended = gain * image_extended
                
            # crude background and segmentation again
            mean, median, std = sigma_clipped_stats(image_extended, sigma=3.0, maxiters=10)
            image_extended_bgsub = image_extended - mean
            mean_init = mean

            threshold = 1.0 * std # 1 sigma
            segmap_extended = detect_sources(image_extended_bgsub, threshold, npixels=100)
            segmap_extended = deblend_sources(image_extended_bgsub, segmap_extended,
                                           npixels=100, nlevels=32, contrast=0.00005,
                                           progress_bar=False)
            segmap_extended = numpy.asarray(segmap_extended)
            
            # now refined background
            sky_distance_map_extended, number_of_sky_pixels = getskymap(image_extended, segmap_extended, pixScale)
            mean, median, std = sigma_clipped_stats(image_extended[sky_distance_map_extended == 1], sigma=3.0, maxiters=10)
            #print('loop factor ', loop_factor, '; # of sky pixels = ', number_of_sky_pixels)
                
            plt.figure()
            plt.imshow(rescale_image(image_extended),cmap="gray")
            ax = plt.gca()
            ax.xaxis.set_visible(False)
            ax.yaxis.set_tick_params(left=False, labelleft=False)
            plt.show()
            ...
            plt.figure()
            plt.imshow(segmap_extended,cmap="gray")
            ax = plt.gca()
            ax.xaxis.set_visible(False)
            ax.yaxis.set_tick_params(left=False, labelleft=False)
            plt.show()
            ...
            plt.figure()
            plt.imshow(sky_distance_map_extended,cmap="gray")
            ax = plt.gca()
            ax.xaxis.set_visible(False)
            ax.yaxis.set_tick_params(left=False, labelleft=False)
            plt.show()
            ...
            # exit loop if image is too big
            # raise error flag and go with sigma clipped estimate for extended image
            loop_factor += 0.5
            if loop_factor > 10 or number_of_sky_pixels > 20000:
                loop = False
...
def main():
    ...
    for i in range(0,30): #len(objid_unique)
        ...
        if str(objid_unique[i]) == '1237648703518998783':
            ...
            for name in images_unique:
                ...
                if os.path.exists(impath):
                    ...
                    process(impath, outfile, exptime, ra, dec, name)
