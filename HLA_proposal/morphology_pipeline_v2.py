...
def get_chi2(residual, sigma):
    ...
    chi2 = numpy.sum( (residual[numpy.isnan(sigma) == False]**2.) / (sigma[numpy.isnan(sigma) == False]**2.) ) / df
    return chi2
...
def process(imname, outfile):
    ...
    # Smooth with simple gaussian kernel
    # 3 pixel FWHM
    """size_wmk = 10  # on each side from the center
    sigma_wmk = 1.0
    wmky, wmkx = numpy.mgrid[-size_wmk:size_wmk+1, -size_wmk:size_wmk+1]
    wmap_kernel = numpy.exp(-(wmkx**2 + wmky**2)/(2.0*sigma_wmk**2))
    wmap_kernel /= numpy.sum(wmap_kernel)
    wmap = convolve(wmap, wmap_kernel)"""
    # This is recommended by Peng on GALFIT fb
    ...
    fit_magnitude = morph.sersic_amplitude
    ...
    outfile.write('{} {} {} {} {} {} {} '.format(SM_flag, SM_flag_sersic, SM_flag_catastrophic, ncombine_flag, unit_flag, gain_flag, number_of_sky_pixels))
    ...
    fit_magnitude = fitted_model.amplitude.value
    ...
    fit_magnitude = fitted_model.amplitude_0.value + fitted_model.amplitude_1.value
    ...
    tBT = fitted_model.amplitude_1.value / fitted_model.amplitude_0.value
...
def main():
    ...
    outfile.write('SM_mag SM_Reff SM_n SM_ellip SM_theta SM_dx SM_dy ')
    outfile.write('SM_flag SM_flag_sersic SM_flag_catastrophic ncombine_flag unit_flag gain_flag skybg_count ')
