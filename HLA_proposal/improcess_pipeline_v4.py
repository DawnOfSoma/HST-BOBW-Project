...
def get_segmap(image_bgsub, threshold):
    ...
    segmap = deblend_sources(image_bgsub, segmap, npixels=100, nlevels=32, contrast=0.2, progress_bar=False)
...
def process(imname, outfile, exptime, ra, dec, imfilename, z, plotlabel):
    ...
    plt.show()
    #plt.savefig('Plots/improcess_ex/' + plotlabel + '_segmap.png', bbox_inches='tight')
    plt.close()
    ...
    image_bgsub = image - mean
    # update segmap with refined background
    #threshold = 3.0 * numpy.sqrt(mean) # 1 sigma above mean poisson noise of bg
    #segmap = get_segmap(image_bgsub, threshold)
    ...
    plt.figure()
    plt.hist(image.flatten(), log=False, bins=100, range=(10**(numpy.log10(mean_init)-2), 10**(numpy.log10(mean_init)+1)))
    plt.axvline(x=mean_init, color='k', label='Initial mean')
    plt.axvline(x=mean, color='r', label='Mean')
    plt.axvline(x=mode, color='g', label='Mode')
    plt.legend()
    plt.show()
    #plt.savefig('Plots/improcess_ex/' + plotlabel + '_hist1.png', bbox_inches='tight')
    plt.close()
    ...
    plt.legend()
    plt.show()
    #plt.savefig('Plots/improcess_ex/' + plotlabel + '_hist2.png', bbox_inches='tight')
    plt.close()
    ...
    plt.legend()
    plt.show()
    #plt.savefig('Plots/improcess_ex/' + plotlabel + '_hist3.png', bbox_inches='tight')
    plt.close()
    ...
    plt.legend()
    plt.show()
    #plt.savefig('Plots/improcess_ex/' + plotlabel + '_hist4.png', bbox_inches='tight')
    plt.close()
    ...
    masked_image[skydm_target == 1] = numpy.nan
    ...
    ax.yaxis.set_tick_params(left=False, labelleft=False)
    plt.show()
    #plt.savefig('Plots/improcess_ex/' + plotlabel + '_img_bgsub.png', bbox_inches='tight')
    plt.close()
    ...
    ax.yaxis.set_tick_params(left=False, labelleft=False)
    plt.show()
    #plt.savefig('Plots/improcess_ex/' + plotlabel + '_img_masked.png', bbox_inches='tight')
    plt.close()
...
def main():
    ...
    outfile = open('CO_HST_improcess_infocat_test.txt', 'w')
        ...
        if i > -99.:
            ...
            for name in images_unique:
                ...
                if os.path.exists(impath):
                    ...
                    process(impath, outfile, exptime, ra, dec, name, z, str(i))
