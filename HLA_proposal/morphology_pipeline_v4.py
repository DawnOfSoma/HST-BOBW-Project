...
def process(imname, outfile, exptime):
    ...
    tempimage = numpy.copy(image)
    tempimage[SM_mask == True] = numpy.nan
    plt.figure()
    plt.imshow(rescale_image(tempimage),cmap="gray")
    ax = plt.gca()
    ax.xaxis.set_visible(False)
    ax.yaxis.set_tick_params(left=False, labelleft=False)
    plt.show()
    ...
    source_morphs = statmorph.source_morphology(image, SM_segmap, weightmap=wmap, psf=psf, mask=SM_mask)
    ...
    outfile.write('{} {} {} {} {} {} {} {} '.format(SM_flag, SM_flag_sersic, SM_flag_catastrophic, ncombine_flag, unit_flag, gain_flag, number_of_sky_pixels, mean))
...
def main():
    ...
    outfile.write('SM_mag SM_Reff SM_n SM_ellip SM_theta SM_dx SM_dy ')
    outfile.write('C A S r20 r50 r80 Gini M20 rpc rpe rhc rhe M I D mag_c mag_e ')
    outfile.write('SM_flag SM_flag_sersic SM_flag_catastrophic ncombine_flag unit_flag gain_flag skybg_count sky ')
    ...
    for i in range(30,31):#len(objid_unique)):
        ...
        #if str(objid_unique[i]) == '1237648703518998792':
